...
function performSearch() {
    ...
    try {
        ...
        resultCount.textContent = result.length + '개의 결과를 찾았습니다.';
        ...
        if (result.length > maxResults) {
            ...
            moreResults.textContent = '더 많은 결과 ' + (result.length - maxResults) + '개가 표시되지 않았습니다.';
            resultdiv.appendChild(moreResults);
        }
    } catch (e) {
        console.error('검색 실행 오류:', e);
        resultdiv.innerHTML = '<p class="results__found">검색 중 오류가 발생했습니다.</p>';
    }
}
