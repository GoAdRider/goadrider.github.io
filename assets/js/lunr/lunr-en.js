...
function displaySearchResults(results, store) {
    ...
    // 결과가 없는 경우 메시지 표시
    if (results.length === 0) {
        searchResults.innerHTML = '<p class="results__found">No results found.</p>';
        return;
    }
...
function initLunr() {
    ...
    // 초기화 시작
    console.log('English search engine initialization');

    // 전역 store 변수가 없거나 비어있으면 기본 데이터로 초기화
    if (!window.store || window.store.length === 0) {
        console.log('No search data, skipping search initialization');
        return;
    }
    ...
    try {
        ...
        if (searchError) {
            ...
            if (errorText) {
                errorText.textContent = 'Error initializing search: ' + error.message;
            }
        }
    }
}
...
function searchSite(query) {
    if (!initComplete) {
        console.log('Search initialization not complete.');
        return;
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('searchDataLoaded', function () {
        console.log('Search data loaded, initializing search engine');
